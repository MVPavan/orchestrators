...
## [INV-05] Codex Safety Hooks Are Wired

- Statement: Codex PreToolUse hooks block dangerous shell commands and hand-edits to generated mirrors.
- Check: `jq -e '.hooks.PreToolUse[] | select(.matcher=="Bash")' .codex/hooks.json >/dev/null && jq -e '.hooks.PreToolUse[] | select(.matcher=="apply_patch|Edit|Write")' .codex/hooks.json >/dev/null`
- Must return: exit 0.
...
- Statement: `.codex/project/` should not contain copied source-project facts.
- Check: `! rg -n "Bo[d]ha|bo[d]ha|Dhri[t]i|Dhṛ[t]i|src/bo[d]ha|docs/design/v_2_[0-9]" .codex/project`
- Must return: exit 0.
...
## [INV-09] Codex Harness Assets Exist

- Statement: Codex-native project assets exist under `.codex/`.
- Check: `test -f .codex/config.toml && test -f .codex/hooks.json && test -f .codex/skills/use-codex/SKILL.md && test -f .codex/agents/planner.toml && test -f .codex/rules/default.rules`
- Must return: exit 0.
- Why it matters: Codex should no longer depend on Claude-only command and skill surfaces.
