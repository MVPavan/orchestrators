...
- **Actor (mandatory, every write):** pass `--actor "<runtime>:<session-or-purpose>"` on *all* `bd create` / `update` / `close` / `dep` / `label`. Examples: `codex:setup`, `codex:migration`, `gemini:<session>`, `cc:<session>`. Stamps `created_by` + each change event; `owner` stays the human.
