...
1. **Select + claim atomically**: `bd ready --parent <epic> --claim --actor "codex:<purpose>"`
...
2. Run `git status` — report uncommitted changes. Commit only when asked or when an active `$run-phases` workflow explicitly grants commit authority.
