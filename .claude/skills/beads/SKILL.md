...
Shared, durable task system for this repo — the source of truth for project work, not markdown TODO files. `bd` is the interface and the table below is self-sufficient for everyday work. Recover context (and the session-close protocol) with `bd prime`; if it prints nothing, check the workspace with `bd where`.
...
Maintenance — on-demand, not per session: `bd doctor` is a no-op in this repo's embedded mode; use the stats/health row above, and `bd preflight --check` before a PR.

## Conventions

- **Actor (mandatory, every write):** pass `--actor "cc:${CLAUDE_CODE_SESSION_ID:0:8}"` on *all* `bd create` / `update` / `close` / `dep` / `label`. Stamps `created_by` + each change event; `owner` stays the human.
...
- **Epic → spec-id (mandatory check):** never create an epic without first checking for a spec. If the user named one, attach it (`--spec-id <path>`); if not, you must ask whether it belongs to a spec — proceed without one only after they decline. `spec_id` does not inherit to children; filter epics by spec with `bd list --spec <prefix>`.
- **Phase work (fork A):** epic = one phase, flat task = one stage (`--parent` → dotted `<epic>.N`); `--spec-id` = workstream anchor (`docs/workstreams/<name>/roadmap.md` or its `README.md`), `--design` = origin brainstorm. Ready-front = `bd ready --parent <epic>` filtered to direct children (`^<epic>\.[^.]+$`). Phase-DONE gate: no non-closed children. Tracking files under `docs/workstreams/**` + the board are **bd-generated — never hand-edit**; update bd, then `BD_RENDER=1 bash scripts/bd-render-tracking.sh`. Full model: [`.beads/beads.md`](../../../.beads/beads.md) → *Phase & workstream integration* and `docs/plans/beads-phase-integration.md`.
...
- **Memory:** durable knowledge → `MEMORY.md`, not `bd remember`. bd holds work items only.
